                ...
                for name in model_part.keys():
                    if is_safetensors:
                        if self.lazy:
                            ...
                            data_gen = lambda data=data: LazyTorchTensor.from_safetensors_slice(
                                data
                            )  # noqa: E731
                        else:
                        ...
                        if self.lazy:
                            data_gen = lambda data=data: LazyTorchTensor.from_eager(
                                data
                            )  # noqa: E731
                        else:
                            data_gen = lambda data=data: data  # noqa: E731
            ...
            if quant_method == "bitnet":
                ...
                for name in self.model_tensors.keys():
                    if name.endswith(".weight_scale_inv"):
                        ...
                        s = self.model_tensors[name]
                        self.model_tensors[weight_name] = (
                            lambda w=w, s=s: dequant_simple(w(), s())
                        )
                        tensors_to_remove.append(name)
...
class TextModel(ModelBase):
    ...
    def _create_vocab_sentencepiece(self):
        ...
        # fix for lapa's transferred tokenizer
        tokenizer_json_file = self.dir_model / "tokenizer.json"
        logger.info(f"Checking for tokenizer.json at {tokenizer_json_file}")
        if tokenizer_json_file.is_file():
            logger.info("Found tokenizer.json, applying fixes if necessary")
            with open(tokenizer_json_file, "r", encoding="utf-8") as f:
                tokenizer_json = json.load(f)
                token_list = tokenizer_json.get("model", {}).get("vocab", {})
                for token, token_id in token_list.items():
                    token_id = int(token_id)
                    token: str = token

                    if token != tokens[token_id].decode("utf-8"):
                        logger.warning(
                            f'replacing token {token_id}: {tokens[token_id].decode("utf-8")!r} -> {token!r}'
                        )
...
class BailingMoeV2Model(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        if hparams["score_function"] == "sigmoid":
            ...
            raise ValueError(
                f"Unsupported score_function value: {hparams['score_function']}"
            )
