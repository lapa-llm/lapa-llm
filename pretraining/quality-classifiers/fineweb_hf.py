...
def main(args):
    dataset = load_dataset(args.dataset_name, split="train", num_proc=64)
    hq_data = dataset.filter(lambda x: x[args.target_column] > 3, num_proc=64)
    lq_data = dataset.filter(lambda x: x[args.target_column] < 2, num_proc=64).take(
        len(hq_data) * 2
    )
    ...
    dataset = dataset.map(
        lambda x: {args.target_column: np.clip(int(x[args.target_column] > 2.5), 0, 1)},
        num_proc=64,
    )
    ...
    dataset = dataset.map(preprocess, batched=True, num_proc=64)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--target_column", type=str, default="fineweb-mixtral-edu-score"
    )
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default="./bert_snowflake_regression",
    )
    parser.add_argument(
        "--output_model_name", type=str, default="lapa-llm/fineweb-mixtral-edu-score"
    )
